...
/* -----------------------------
   ...
   ----------------------------- */
const defaultUnitSchema = () => ({ cantidadLunesViernes: 0, cantidadSabado: 0, cantidadDomingo: 0, cantidadFestivo: 0 });
const clamp = (n) => Math.max(0, Number.isFinite(n) ? n : 0);
const parseNumber = (e) => { const v = parseInt(e.target.value, 10); return Number.isNaN(v) ? 0 : clamp(v); };
...
export default function ConfigurarUnidadesAportantesModal({
    ...
    const [globalCfg, setGlobalCfg] = useState({ cantidadLunesViernes: 0, cantidadSabado: 0, cantidadDomingo: 0, cantidadFestivo: 0, finde: 0 });
    ...
    const updateUnidadField = (idUnidad, field, value) => {
        setConfigPorUnidad((prev) => ({
            ...prev,
            [idUnidad]: { ...(prev[idUnidad] || defaultUnitSchema()), [field]: clamp(value) },
        }));
    };
    ...
    const agregarVisibles = () => {
        ...
        setConfigPorUnidad((prev) => {
            const next = { ...prev };
            unidadesFiltradas.forEach((u) => { if (!next[u.id]) next[u.id] = defaultUnitSchema(); });
            return next;
        });
    };

    const quitarSeleccion = (idUnidad) => {
        setSeleccionadas((prev) => { const next = { ...prev }; delete next[idUnidad]; return next; });
    };
        ...
        setConfigPorUnidad((prev) => {
            ...
            for (const id of fuente) {
                ...
                next[id] = agruparFinDeSemana
                    ? { ...actual, cantidadLunesViernes: clamp(globalCfg.cantidadLunesViernes), cantidadSabado: clamp(globalCfg.finde), cantidadDomingo: clamp(globalCfg.finde), cantidadFestivo: clamp(globalCfg.cantidadFestivo) }
                    : { ...actual, cantidadLunesViernes: clamp(globalCfg.cantidadLunesViernes), cantidadSabado: clamp(globalCfg.cantidadSabado), cantidadDomingo: clamp(globalCfg.cantidadDomingo), cantidadFestivo: clamp(globalCfg.cantidadFestivo) };
            }
            return next;
        });
    };
        ...
        <Modal show={show} onHide={onHide} fullscreen scrollable>
                ...
                <Row className="g-2">
                    ...
                    <Col md={8} lg={9}>
                        ...
                        <div className="mb-2 p-2 border rounded-3">
                            <Row className="g-2 align-items-end">
                                <Col md={4} sm={6} xs={12}>
                                    ...
                                    <Form.Select size="sm" value={regionId} onChange={(e) => setRegionId(e.target.value)}>
                                ...
                                <Col md={5} sm={6} xs={12}>
                                    ...
                                    <Form.Select size="sm" value={prefecturaId} onChange={(e) => setPrefecturaId(e.target.value)} disabled={!regionId}>
                                ...
                                <Col md={3} xs={12}>
                                    ...
                                    <InputGroup size="sm">
                                        ...
                                        <Form.Control placeholder="Nombre de unidad" value={query} onChange={(e) => setQuery(e.target.value)} disabled={!prefecturaId} />
                                    </InputGroup>
                                </Col>
                            </Row>
                        </div>
                        ...
                        <div className="mb-2 p-2 border rounded-3">
                            <div className="d-flex align-items-center justify-content-between mb-2">
                                ...
                                <Form.Check type="switch" id="switch-usar-global" label="Habilitar" checked={usarGlobal} onChange={(e) => setUsarGlobal(e.target.checked)} />
                            </div>

                            <Row className="g-2">
                                <Col md={3} sm={6} xs={6}>
                                    <Form.Label className="small mb-1">Lun–Vie</Form.Label>
                                    <Form.Control size="sm" type="number" min={0} value={globalCfg.cantidadLunesViernes} onChange={(e) => setGlobalCfg({ ...globalCfg, cantidadLunesViernes: parseNumber(e) })} disabled={!usarGlobal} />
                                </Col>

                                <Col md={3} sm={6} xs={6}>
                                    <div className="d-flex align-items-center justify-content-between">
                                        <Form.Label className="small mb-1">Fin de semana</Form.Label>
                                        <Form.Check type="switch" id="switch-weekend-group" label="Agrupar" checked={agruparFinDeSemana} onChange={(e) => setAgruparFinDeSemana(e.target.checked)} disabled={!usarGlobal} />
                                    </div>
                                    <Form.Control size="sm" type="number" min={0} className="mt-1" value={globalCfg.finde} onChange={(e) => setGlobalCfg({ ...globalCfg, finde: parseNumber(e) })} disabled={!usarGlobal || !agruparFinDeSemana} placeholder="Sáb+Dom" />
                                </Col>
                                ...
                                {!agruparFinDeSemana && (
                                    <>
                                        <Col md={3} sm={6} xs={6}>
                                            ...
                                            <Form.Control size="sm" type="number" min={0} value={globalCfg.cantidadSabado} onChange={(e) => setGlobalCfg({ ...globalCfg, cantidadSabado: parseNumber(e) })} disabled={!usarGlobal} />
                                        </Col>
                                        <Col md={3} sm={6} xs={6}>
                                            <Form.Label className="small mb-1">Domingo</Form.Label>
                                            <Form.Control size="sm" type="number" min={0} value={globalCfg.cantidadDomingo} onChange={(e) => setGlobalCfg({ ...globalCfg, cantidadDomingo: parseNumber(e) })} disabled={!usarGlobal} />
                                        </Col>
                                    </>
                                )}
                                ...
                                <Col md={3} sm={6} xs={6}>
                                    ...
                                    <Form.Control size="sm" type="number" min={0} value={globalCfg.cantidadFestivo} onChange={(e) => setGlobalCfg({ ...globalCfg, cantidadFestivo: parseNumber(e) })} disabled={!usarGlobal} />
                                </Col>
                            </Row>

                            <div className="mt-2 d-flex gap-2 flex-wrap">
                                <Button size="sm" variant="outline-primary" onClick={() => aplicarGlobal("visibles")} disabled={!usarGlobal || !prefecturaId}>
                                    Aplicar a visibles ({unidadesFiltradas.length})
                                </Button>
                                <Button size="sm" variant="outline-success" onClick={() => aplicarGlobal("seleccionadas")} disabled={!usarGlobal || totalSeleccionadas === 0}>
                        ...
                        <div className="p-2 border rounded-3">
                            ...
                            <Table responsive hover size="sm" className="align-middle mb-2">
                                ...
                                {pageItems.map((u) => {
                                    ...
                                    return (
                                        <tr key={u.id}>
                                            ...
                                            <td>
                                                <Form.Control size="sm" type="number" min={0} value={cfg.cantidadLunesViernes} onChange={(e) => updateUnidadField(u.id, "cantidadLunesViernes", parseNumber(e))} />
                                            </td>
                                            <td>
                                                <Form.Control size="sm" type="number" min={0} value={cfg.cantidadSabado} onChange={(e) => updateUnidadField(u.id, "cantidadSabado", parseNumber(e))} />
                                            </td>
                                            <td>
                                                <Form.Control size="sm" type="number" min={0} value={cfg.cantidadDomingo} onChange={(e) => updateUnidadField(u.id, "cantidadDomingo", parseNumber(e))} />
                                            </td>
                                            <td>
                                                <Form.Control size="sm" type="number" min={0} value={cfg.cantidadFestivo} onChange={(e) => updateUnidadField(u.id, "cantidadFestivo", parseNumber(e))} />
                                            </td>
                                            <td className="text-end">
                                                <Button size="sm" variant={inCart ? "outline-danger" : "outline-primary"} onClick={() => toggleSeleccion(u)}>
                                ...
                                {prefecturaId && unidadesFiltradas.length === 0 && (
                                    <tr>
                                        <td colSpan={6} className="text-center text-muted py-4">Sin resultados para este filtro.</td>
                                    </tr>
                                )}
                                </tbody>
                            </Table>
                            ...
                            <div className="d-flex justify-content-end">
                                <Button size="sm" variant="outline-secondary" onClick={agregarVisibles} disabled={!prefecturaId || unidadesFiltradas.length === 0}>
                    ...
                    <Col md={4} lg={3}>
                        <div className="p-2 border rounded-3 h-100 d-flex flex-column" style={{ minHeight: 300 }}>
                                        ...
                                        return (
                                            <div key={idUnidad} className="border rounded-3 p-2 position-relative">
                                                <div className="d-flex align-items-start justify-content-between">
                                                    <div>
                                                        <div className="fw-semibold small">{meta.siglasUnidad || idUnidad}</div>
                                                        <div className="text-muted small">{meta.prefecturaId} · Región {meta.regionId}</div>
                                                    </div>
                            ...
                            <div className="mt-2 d-flex gap-2 flex-wrap">
                                <Button size="sm" variant="outline-danger" onClick={limpiarCarrito} disabled={totalSeleccionadas === 0}>
                                    Limpiar selección
                                </Button>
                            </div>
                        </div>
            ...
            <Modal.Footer className="py-2 d-flex justify-content-between">
                ...
                <div className="d-flex gap-2">
                    ...
                    <Button size="sm" variant="primary" onClick={handleGuardar} disabled={Object.keys(seleccionadas).length === 0}>
