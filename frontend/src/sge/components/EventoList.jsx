...
import { MapPin, Calendar, Flame, AlertOctagon, Activity, Trash2 } from 'lucide-react';
...
const EventoList = () => {
  ...
  const fetchEventos = async () => {
    try {
      const response = await sgeApi.get('/eventos');
  ...
  const handleDelete = async (id) => {
    if (window.confirm("¿Estás seguro de que quieres eliminar este evento? Esta acción no se puede deshacer.")) {
      try {
        await sgeApi.delete(`/eventos/${id}`);
          ...
          <tbody className="divide-y divide-slate-100">
            {eventos.map((evento) => (
              <tr key={evento.id} className="hover:bg-slate-50 transition-colors group">
                ...
                <td className="px-6 py-4 text-right">
                  <Link to={`/sge/eventos/${evento.id}`} className="inline-flex items-center justify-center px-4 py-2 border border-slate-300 shadow-sm text-sm font-medium rounded-md text-slate-700 bg-white hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 mr-2">
                    Gestionar
                  </Link>
                  <button
                    onClick={() => handleDelete(evento.id)}
                    className="inline-flex items-center justify-center px-2 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-red-600 bg-red-50 hover:bg-red-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500"
                    title="Eliminar"
                  >
                    <Trash2 size={18} />
                  </button>
