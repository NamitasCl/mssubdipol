...
const EventReport = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // Fetch Event
                const resEvento = await sgeApi.get(`/eventos/${id}`);
                setEvento(resEvento.data);

                // Fetch All Deployments for this Event
                const resDespliegues = await sgeApi.get(`/eventos/${id}/despliegues`);
                ...
                for (let depId of desplieguesIds) {
                    const resAsig = await sgeApi.get(`/asignaciones/despliegue/${depId}`);
    ...
    return (
        <div className="bg-gray-100 min-h-screen text-gray-900 font-sans print:bg-white">
            ...
            <div className="bg-white shadow p-4 mb-6 print:hidden flex justify-between items-center">
                <Link to="/sge" className="flex items-center gap-2 text-gray-600 hover:text-blue-600 font-medium">
