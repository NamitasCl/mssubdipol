...
const DespliegueDetail = () => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        const resDes = await axios.get(`/api/despliegues/${id}`);
        ...
        const resAsig = await axios.get(`/api/asignaciones/despliegue/${found.id}`);
  ...
  const handleUpdateRequirements = async () => {
      try {
          ...
          await axios.put(`/api/despliegues/${despliegue.id}`, updated);
  ...
  const handleProrrogaMismoPeriodo = async () => {
      try {
          const res = await axios.put(`/api/despliegues/${despliegue.id}/prorroga`, { mismoPeriodo: true });
  ...
  const handleProrrogaNewDate = async () => {
      ...
      try {
          const res = await axios.put(`/api/despliegues/${despliegue.id}/prorroga`, { 
              mismoPeriodo: false,
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-4">
          <Link to="/despliegues" className="flex items-center gap-2 text-gray-600 hover:text-blue-600">
            <ArrowLeft size={18} /> Volver al Monitor
          </Link>
          
          {despliegue.evento && (
              <Link 
                to={`/eventos/${despliegue.evento.id}/reporte`} 
        ...
        <div className="lg:w-1/3 space-y-4">
            ...
            <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-indigo-500">
                ...
                <div className="flex justify-between items-end">
                    ...
                    <Link to={`/despliegues/${despliegue.id}/asignar`} className="text-indigo-600 text-sm font-semibold hover:underline">
                        + Asignar
                    </Link>
                </div>
