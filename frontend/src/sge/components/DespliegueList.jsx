...
const DespliegueList = () => {
    ...
    const fetchData = async () => {
        try {
            const res = await sgeApi.get('/despliegues');
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await sgeApi.delete(`/despliegues/${id}`);
            fetchData();
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {despliegues.map(d => (
                                <tr key={d.id} className="hover:bg-blue-50/50 transition-colors group text-sm">
                                    ...
                                    <td className="px-6 py-4">
                                        {d.evento ? (
                                            <Link to={`/sge/eventos/${d.evento.id}`} className="font-semibold text-primary-700 hover:text-primary-900 hover:underline flex items-center gap-1">
